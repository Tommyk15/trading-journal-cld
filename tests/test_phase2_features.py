...
async def test_analytics_service_win_rate(db_session: AsyncSession):
    ...
    trades = [
        Trade(
            ...
            closed_at=datetime.utcnow() - timedelta(days=i-1),
    ...
    for i, strategy in enumerate(strategies):
        for j in range(3):
            trade = Trade(
                ...
                status="CLOSED",
                opened_at=datetime.utcnow() - timedelta(days=i*3+j),
                closed_at=datetime.utcnow() - timedelta(days=i*3+j-1),
...
async def test_performance_metrics_cumulative_pnl(db_session: AsyncSession):
    ...
    for i in range(5):
        trade = Trade(
            ...
            strategy_type="Single",
            status="CLOSED",
            opened_at=datetime.utcnow() - timedelta(days=5-i),
            closed_at=datetime.utcnow() - timedelta(days=4-i),
